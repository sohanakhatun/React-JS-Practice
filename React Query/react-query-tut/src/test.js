  ...
  customElements.define(
    ...
    class ProductForm extends HTMLElement {
      constructor() {
        ...
        this.buyNowbtn = this.querySelector(".sr-headless-checkout-dummy");
        this.buyNowbtn?.addEventListener("click", (e) => {
          e.preventDefault();
          this.clickEventForBuyNowBtn(e);
        });
      }

      clickEventForBuyNowBtn() {
        let isVariantSelected = window.location.href.includes("?variant=")
          ? true
          : false;
        if (!isVariantSelected) {
          // Show Error
          this.handleErrorMessage("Please select a variant first");
          return;
        }
      }
      ...
      onSubmitHandler(evt) {
        ...
        let isVariantSelected = window.location.href.includes("?variant=")
          ? true
          : false;
        let isPreselectVariantEnabled =
          this.submitButton?.dataset.enabledPreselect;
        if (isPreselectVariantEnabled == "false") {
          if (!isVariantSelected) {
            // Show Error
            this.submitButton.classList.remove(".no-active-variant");
            this.handleErrorMessage("Please select a variant first");
            return;
          } else {
            this.submitButton.querySelector("span").innerHTML = "Add to cart";
          }
        }
      ...
      checkForBundleProducts() {
        window
          .fetch(`${window.routes.cart_url}.json`)
          .then((response) => response.json())
          .then((cart) => {
            for (const item of cart.items) {
              if (item.has_components == true) {
                window.__showShopifyByTag = true;
                break;
              } else {
                window.__showShopifyByTag = false;
              }
            }
          });
      }
